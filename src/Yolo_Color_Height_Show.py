            ...
            try:
                ...
                for result in results:
                    ...
                    if label == 'person':
                        ...
                        person_color_bgr = color_NAME2BGR(person_color)

                        frame = cv2.rectangle(frame, tl, br, person_color_bgr, 4)
                        ...
                        # 객체 정보의 이름& 키 이미지에 표시
                        frame = cv2.putText(frame, label, (tl[0], tl[1] - 50), cv2.FONT_ITALIC, 1, (0, 0, 0), 2)
                        frame = cv2.putText(frame, "height : {}".format(round(char_hei, 2)), (tl[0], tl[1] - 25), cv2.FONT_ITALIC, 1,(0, 0, 0), 2)
                        frame = cv2.putText(frame,"Color : {}".format(person_color),tl, cv2.FONT_ITALIC, 1, (0, 0, 0), 2)
                    ...
                    if label in ['car','truck']:
                        ...
                        frame = cv2.rectangle(frame, tl, br, car_color_bgr, 4)

                        # 객체 정보의 이름& 키 이미지에 표시
                        frame = cv2.putText(frame, 'car', (tl[0], tl[1] - 25), cv2.FONT_ITALIC, 1, car_color_bgr, 2)
                        frame = cv2.putText(frame, "car_color : {}".format(car_color), tl, cv2.FONT_ITALIC, 1, car_color_bgr, 2)
...
ans = yolo_color("F:/socool/socool/webserver/src/static/videos/a0003_20190305.mp4")
# ans = yolo_color("D:/Acorn/PythonWork/DL_3D_AND_YOLO/darkflow/sample_video/a0003_20190306.mp4")
